...
for session in sessions:
    ...
    for poster in glob.glob(glob.escape(session) + '/*'):
        ...
        cmd = magick_convert + [poster,"-resize","320x450!","./output/tmp/{}.png".format(name)]
        p = subprocess.Popen(cmd)
        p.wait()
        pass
    cmd = magick_convert + poster_array + ["+append","./output/tmp/lineup.png"]
    p = subprocess.Popen(cmd)
    p.wait()

    cmd = magick_convert + ["./output/posters.png","-gravity","center","-fill","white","-pointsize","40","-font",font,"-annotate","+480+40","This session:\n" + " & ".join(current), "./output/tmp/text.png"]
    p = subprocess.Popen(cmd)
    p.wait()

    cmd = magick_convert + ["./output/tmp/text.png","./output/tmp/lineup.png", "-gravity","center","-geometry","+480+315","-composite", "./output/splash/{}.png".format("+".join(current))]
    p = subprocess.Popen(cmd)
    p.wait()
